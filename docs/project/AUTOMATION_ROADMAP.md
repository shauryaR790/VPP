# V++ North Star — Automation Roadmap v1.3 → v2.0.0
...
8. Syntax changes only when stdlib design fails in real programs (v1.7+ gate).
...
|---------|------|---------------|
| **v1.3** | Automation foundation | Typed process I/O, env, dirs, structured errors, timeouts, logging primitives |
| **v1.4** | Workflows | Parallel/sequential composition, retry, cancel, failure propagation (stdlib first) |
| **v1.5** | Program-to-program integrations | Git, HTTP, Docker, archives — consistent typed stdlib, not compiler keywords |
| **v1.6** | Deployable automation | Config, secrets, profiles, structured logs, CI-friendly exit/JSON output |
| **v1.7** | Intelligent workflow execution | Dependency graphs, caching, incremental runs, safe auto-parallelization |
| **v1.8** | Performance + ecosystem | Runtime efficiency, portable integrations, tooling for third-party modules |
| **v1.9** | Stabilization | Benchmarks, diagnostics, docs, reliability — no random features |
| **v2.0.0** | The automation language | BUILD→TEST→PACKAGE→DEPLOY→VERIFY as one native program; compiler understands automation |

**The differentiator is v1.7 → v2.0:** not “we have Docker APIs,” but **the compiler/runtime understands dependencies, failures, caching, and concurrency.**
...
| Exists | Gap |
|--------|-----|
| `Result`, `Option`, `match`, structs, modules | I/O does not use `Result` |
| `std.process.run(cmd) -> int` (shell string) | No argv, stdout/stderr, timeout, cwd |
| `std.fs` read/write/exists | No directories |
| JSON string validate/escape | No typed JSON values |
| Interpreter catchable errors | Native runtime `exit(1)` on I/O failure |
...
| Errors | `AutomationError` struct in stdlib; APIs return `Result` |
| Logging | `log.info/warn/error` → structured lines (stderr) |
| Timeouts | `ProcessOptions.timeout_ms` |
| HTTP | Basic GET/POST via runtime (v1.3.1 if lib integration slips) |

### Do not ship in v1.3

- Workflow syntax (`parallel {}`, `retry N {}`)
- Git/Docker wrappers (v1.5)
- YAML config loader (v1.6)

### Files touched

- `runtime/vpp_runtime.c` — process pipes, env, dir
- `src/builtins/mod.rs`, `src/types/check.rs`, `src/interp/mod.rs`, `src/codegen/emit.rs`
- `std/automation.vpp`, `std/process.vpp`, `std/env.vpp`, `std/log.vpp`, extend `std/fs.vpp`
...
Fixed thread pool; no async/await in v1.4.

---

## v1.5 — Program-to-program integrations
...
## v1.7 — Intelligent workflow execution

Review v1.3–v1.6 real programs. Ship only proven features:
...
SPEC v2.0 may add proven language features from v1.7 experiments.
...
## v1.3 implementation checklist (active)

- [x] This roadmap document  
- [ ] `std/automation.vpp` — shared types  
- [ ] Process exec with argv + stdout/stderr + timeout  
- [ ] Environment builtins  
- [ ] Directory builtins  
- [ ] `std/log.vpp`  
- [ ] `tests/automation.rs`  
- [ ] HTTP (track v1.3.1 if needed)
